...
export default function ProductDetail() {
  ...
  if(!p) return <Navigate to="/v1-1/products" replace/>;
  const others=PRODUCTS.filter(x=>x.id!==id);
  return (
    <main className="pd-page">
      <div className="pd-bread"><div className="pd-bread-inner">
        <Link to="/v1-1/">Home</Link><span>›</span><Link to="/v1-1/products">Products</Link><span>›</span><span>{p.name}</span>
      </div></div>
      <div className="pd-inner">
        <div className="pd-main">
          ...
          <div className="pd-body">
            ...
            <div className="pd-cta">
              <Link to="/v1-1/contact" className="btn-primary">Request Technical Data Sheet</Link>
              <Link to="/v1-1/contact" className="btn-outline">Enquire About This Product</Link>
            </div>
          </div>
        </div>
        <aside className="pd-sidebar">
          <div className="pd-sb-card">
            ...
            {others.map(x=>(
              <Link to={`/v1-1/products/${x.id}`} key={x.id} className="pd-sp-link">
          ...
          <div className="pd-sb-card pd-sb-dark">
            ...
            <Link to="/v1-1/contact" className="btn-primary" style={{marginTop:'12px',display:'inline-block'}}>Contact Us</Link>
          </div>
        </aside>
      </div>
    </main>
